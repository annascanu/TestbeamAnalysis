...
int main(int argc, char* argv[]) 
{
    ...
    std::filesystem::create_directories("/home/riccardo-speziali/Scrivania/git/TestbeamAnalysis/july2026/eventbuilder/run" + std::to_string(run_number));
    TString filename_feb1 = "/home/riccardo-speziali/Scrivania/git/TestbeamAnalysis/sampic2root_july2026/root_file/run" + std::to_string(run_number) + "/sampic_run1_feb0_Corr.root";
    TString filename_feb3 = "/home/riccardo-speziali/Scrivania/git/TestbeamAnalysis/sampic2root_july2026/root_file/run" + std::to_string(run_number) + "/sampic_run1_feb1_Corr.root";
    ...
    TString output_filename_feb1 = "/home/riccardo-speziali/Scrivania/git/TestbeamAnalysis/july2026/eventbuilder/run" + std::to_string(run_number) + "/ordered_feb1.root";
    TFile *output_file = new TFile(output_filename_feb1.Data(), "RECREATE");
    TTree *output_tree = new TTree("eventbuilding", "eventbuilding");
    output_tree->Branch("Cell0TimeStamp_FEB1", &rec1.Cell0TimeStamp,"Cell0TimeStamp_FEB1/D");
    ...
    TString output_filename_feb3 = "/home/riccardo-speziali/Scrivania/git/TestbeamAnalysis/july2026/eventbuilder/run" + std::to_string(run_number) + "/ordered_feb3.root";
    TFile *output_file3 = new TFile(output_filename_feb3.Data(), "RECREATE");
    TTree *output_tree3 = new TTree("eventbuilding", "eventbuilding");  

    output_tree3->Branch("Cell0TimeStamp_FEB3", &rec3.Cell0TimeStamp,"Cell0TimeStamp_FEB3/D");
    ...
    for (Long64_t i = 0; i < nentries_feb3; i++) 
    {
        ...
        if(rec3.channel == 32) {i++; continue;} // skip MCP channels
        EntryRef e;
        e.time  = rec3.Cell0TimeStamp;
        e.entry = i;
    ...
    for (int i=0; i<100; i++) 
    {
        ...
        if (rec1.Cell0TimeStamp < prev)
            std::cout << "Not ordered!" << std::endl;
        prev = rec1.Cell0TimeStamp;
    }
