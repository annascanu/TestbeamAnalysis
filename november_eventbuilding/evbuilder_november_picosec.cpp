...
struct matchedEvent 
{   
    double Cell0TimeStamp_corr; // add for corrected timestamp
    int channel;
...
int main(int argc, char* argv[]) 
{
    ...
    // reserve space for vectors to avoid reallocations (optional, but can improve performance)
    channel_picosec.reserve(5);
    tot_picosec.reserve(5);
    waveform_picosec.reserve(5);
    cell0.reserve(5);

    // ROOT files in local and/or on lxplus
    ...
    // various checks on files and trees
    ...
    double time_window = 50.0; // time window in ns
    Long64_t j1 = 0, j3 = 0;   // global index for FEB1 and FEB3 trees
    ...
    for (Long64_t i = 0; i < nentries_matching; i++) 
    {
        ...
        {
            cout << "Warning: MCP timestamps not in order at entry n. " << i << endl;
        }
        ...
        while (j1 < nentries_feb1) 
        {
            ...
            if (dt > time_window) 
            { 
                break; // over time window, stop searching
            }

            // Valid hit within time window
        ...
        while (j3 < nentries_feb3) 
        {
            ...
            // for valid hit within time window
            built.Cell0timeSTamp_PICOSEC[hitxevent] = rec3.Cell0TimeStamp_corr;
            built.chanel_PICOSEC[hitxevent] = rec3.channel + 64; // FEB3 channels from 64 to 127
        ...
        if (i % 1000 == 0) 
        {
            cout << "Events left: " << nentries_matching - i << " | Hits this event: " << hitxevent << "\r" << flush;
        }   

        if(t_mcp> 8.75e12 && hitxevent>0) 
        { // example of debug printout for a specific event with hits in FEB1 and/or FEB3
    ...
    // graphical check of occupancy after event building
    int fakeargc = 0;
    char** fakeargv = nullptr;
    TApplication app("app", &fakeargc, fakeargv);

    // Histogram for channel occupancy
    TH1I *hOcc = new TH1I("hOcc", "Channel Occupancy after EventBuilding;Channel;Counts", 128, 0, 128);

    // loop on MCP events (event building)
