...
void InitializeHistograms(Histograms &h) 
{
    ...
    h.htriple1 = new TH1F("htriple1", "Pulse amplitude for first detector", 1000, 0, 1.5);
    h.htriple2 = new TH1F("htriple2", "Pulse amplitude for second detector", 1000, 0, 1.5);
    ...
    //h.mapDet3 = new TH2F("hitmapdet3", "Hit map for detector 3;x;y", 30, -5, 5, 30, -5, 5);
    //h.mapDet3 = new TH2F("hitmapdet3", "Hit map for detector 3;x;y", 30, -5, 5, 30, -5, 5);
    h.mapdet1_afterselection = new TH2F("hitmapdet1_afterselection", "Hitmap detector 1 after selection;x;y", 10, 0, 10, 10, 0, 10);
    h.mapdet2_afterselection = new TH2F("hitmapdet2_afterselection", "Hitmap detector 2 after selection;x;y", 10, 0, 10, 10, 0, 10);
    h.mapdet3_afterselection = new TH2F("hitmapdet3_afterselection", "Hit map for detector 3 after selection;x;y", 30, -5, 5, 30, -5, 5);
    // Graphs
...
{
    int x_pico, y_pico;
    cout << "Select the coordinates(x=-1, y=-1 == no selection): \n " << endl;
    cin >> x_pico >> y_pico;
    cout << "Hai inserito: " << x_pico << ", " << y_pico << endl;
    ...
    for (Long64_t i = 0; i < nentries; i++) 
    {
        ...
        if(true)
        {
            double ampFEB[MAXPULSES];
            double cfd10[MAXPULSES];
            double cfd20[MAXPULSES];
            double cfd30_det1[MAXPULSES];
            double cfd30_det2[MAXPULSES];

            double cfd50[MAXPULSES];
            double cfd60[MAXPULSES];
            double base[MAXPULSES];
            double peak_time[MAXPULSES];
            double risetime[MAXPULSES];
            double integral[MAXPULSES];
            float arrayDetector[MAXPULSES];
                double arrayCell0TimeStamp_det1[MAXPULSES];
                double arrayCell0TimeStamp_det2[MAXPULSES];
            int arrayhitevent[3]={0,0,0};

            
            for (int j = 0; j < b.npulses; j++) 
            {
                
                float det = b.Detector[j];
                //cout << "Board: " << det << endl;
               ...
               // cout << "cavallo"<<endl;
                ...
                ampFEB[j] = b.pulses_amplitude[j];
                cfd10[j] = b.pulses_time_cfd10[j];
                cfd20[j] = b.pulses_time_cfd20[j];
                //cfd30[j] = b.pulses_time_cfd30[j];
                cfd50[j] = b.pulses_time_cfd50[j];
                cfd60[j] = b.pulses_time_cfd60[j];
                base[j] = b.Baseline[j];
                peak_time[j] = b.pulses_peak_time[j];
                risetime[j] = b.pulses_rise_time[j];
                integral[j] = b.pulses_integral[j];
                arrayDetector[j] = b.Detector[j];
                //if(i%1000==0) cout << "Detector: " << arrayDetector[j] << endl;
                ...
                bool hit_on_selected_channel = false;
            for (int j = 0; j < b.npulses; j++) {
                bool doppie=false;
            if (x_pico>=0) 
            {
            
            //hit_on_selected_channel = true;
                

               // for (int j = 0; j < b.npulses; j++) 
                //{
                    if ((b.pulses_channel_x[j] == cx && b.pulses_channel_y[j] == cy) && (b.Detector[j]==1 || b.Detector[j]==2)) 
                    {
                        if (arrayhitevent[0] == 1 && arrayhitevent[1]==1) doppie=true; 
                        
                    }
                //}
                /*if(doppie){
                    cout << "Selected event with one hit on each FEB at coordinates (" << cx << ", " << cy << ")" << endl;
                }*/
        ...
        cout << "Hits on event " << i << ": FEB1 = " << arrayhitevent[0] << ", FEB2 = " << arrayhitevent[1] << ", FEB3 = " << arrayhitevent[2] << endl;
            // After the channel selection condition, otherwise it's useless
            //cut di riccardo 
        //for(int j=0; j<b.npulses; j++) {
            if(b.Detector[j]==1 && (b.pulses_channel_x[j] == cx && b.pulses_channel_y[j] == cy)&& (arrayhitevent[0] == 1 && arrayhitevent[1]==1)){
            h.hTime20->Fill(cfd20[j]);
            //h.hTime30->Fill(cfd30[j]);
            h.hTime50->Fill(cfd50[j]);
            h.hTime60->Fill(cfd60[j]);
            cfd30_det1[j] = b.pulses_time_cfd30[j];
            h.hrisetime1->Fill(risetime[j]);
            arrayCell0TimeStamp_det1[j] = b.Cell0TimeStamp[j];
            h.mapdet1_afterselection->Fill(b.pulses_channel_x[j], b.pulses_channel_y[j]);
            }
            if(b.Detector[j]==2 && (b.pulses_channel_x[j] == cx && b.pulses_channel_y[j] == cy) && (arrayhitevent[0] == 1 && arrayhitevent[1]==1)){
           // h.hTime30_2->Fill(cfd30[j]);
            h.hrisetime2->Fill(risetime[j]);
            cfd30_det2[j] = b.pulses_time_cfd30[j];
            arrayCell0TimeStamp_det2[j] = b.Cell0TimeStamp[j];
            ...
            // Time differences
            double time30_12 = (cfd30_det1[j]+arrayCell0TimeStamp_det1[j]*1000) - (cfd30_det2[j]+arrayCell0TimeStamp_det2[j]*1000);
            //cout << "Time difference (det1 - det2): " << time30_12 << endl;
            ...
            // Fill graphs
            double peso = 1.0;
            h.timevsamplitude->SetPoint(h.timevsamplitude->GetN(), ampFEB[0], cfd50[0]);
            h.timevsamplitude2->SetPoint(h.timevsamplitude2->GetN(), ampFEB[1], cfd50[1]);
            h.prof->Fill(ampFEB[0], cfd50[0]);
            h.prof2->Fill(ampFEB[1], cfd50[1]);
            h.timevsintegral->SetPoint(h.timevsintegral->GetN(), integral[0], cfd50[0]);
            ...
            h.cfdvschannel1->SetPoint(h.cfdvschannel1->GetN(), b.Channel[0], cfd50[0]);
        //}
...
/*   // Polya function
    ...
    */
    ...
    TF1 *f_gaus = new TF1("f_gaus", "gaus", -100, 50);
...
void CreateCanvases(Histograms &h) 
{
    ...
    /*TCanvas *c_prof = new TCanvas("c_prof", "Time vs Amplitude - FEB0 / FEB1", 800, 600);
    c_prof->cd();
    c_prof->SetGrid();*/
    ...
    TCanvas *c_q = new TCanvas("c_q", "Time vs Integral", 900, 700);
    c_q->SetGrid();

    h.timevsamplitude->SetMarkerStyle(20);
    ...
    h.timevsamplitude->SetLineWidth(0);
    h.timevsamplitude->Draw("AP");  // A=assi, P=punti
    c_q->SaveAs("TimeVsAmplitude.pdf");

    delete c_q;

    TCanvas *c_q2 = new TCanvas("c_q22", "Time vs Integral", 900, 700);
    c_q2->SetGrid();

    h.timevsamplitude2->SetMarkerStyle(20);
    ...
    //h.timevsamplitude2->ComputeRange();
    h.timevsamplitude2->SetLineWidth(0);
    h.timevsamplitude2->Draw("AP");  // A=assi, P=punti
                 ...
                 const vector<vector<double>> &tabella2, FitResults &fit)
{
    ...
    h.mapdet3_afterselection->Write();
...
void TimeRes(TTree *tree, TreeBranches &b, TTree *tree_correction)
{
    ...
    for (Long64_t i = 0; i < nentries; i++)
    {
        ...
        if (b.npulses==2 && (b.Detector[0] != 0 || b.Detector[1] != 1))
            continue;
        if (x_pico>=0)
            continue;
...
{
    int ilcanale;
    cout<<"Select the channel(1: selecting the center, 2; slectind the 2nd one 3: not yet coded):   \n " << endl;
    cin>> ilcanale;
    ...
    for (Long64_t i = 0; i < nentries; i++) 
    {
        ...
        if (b.Detector[2] == 1 )
        {
            ...
            for (int j = 0; j < b.npulses; j++) 
            {
                ...
                int centerx[16] = {3,3,4,4,2,2,2,2,3,4,5,5,5,5,4,3};
                int centery[16] = {4,5,4,5,2,3,4,5,5,5,5,4,3,2,2,2};
                ...
                if(ilcanale==1)
                {
                    ...
                    {
                        if (det==0)
                            if ( (ch_x[0]==centerx[k]) && (ch_y[0]==centery[k]) )
                                h.mapdet1_afterselection->Fill(ch_x[0], ch_y[0]);
                        if (det==1)
                            if ( (ch_x[1]==centerx[k]) && (ch_y[1]==centery[k]) )
...
void test_events(TTree *tree, TreeBranches &b, Histograms &h) 
{
    int x_pico, y_pico;
    cout << "Select the coordinates(x=-1, y=-1 == no selection): \n " << endl;
    cin >> x_pico >> y_pico;
  ...
  for (Long64_t i = 0; i < nentries; i++) 
{
    ...
    // =========================
    // SELEZIONE DOPPIE (livello evento)
    // =========================
    bool doppie = (arrayhitevent[0] == 1 && arrayhitevent[1] == 1);
   ...
   // h.hQ2->Fill(b.pulses_integral[idx2]);
    ...
    h.mapdet1_afterselection->Fill(
        b.pulses_channel_x[idx1],
        b.pulses_channel_y[idx1]);

    h.mapdet2_afterselection->Fill(
        b.pulses_channel_x[idx2],
        b.pulses_channel_y[idx2]);
...
void triple_events(TTree *tree, TreeBranches &b, Histograms &h) 
{
    int x_pico, y_pico;
    cout << "Select the coordinates(x=-1, y=-1 == no selection): \n " << endl;
    cin >> x_pico >> y_pico;
  ...
  for (Long64_t i = 0; i < nentries; i++) 
{
    ...
    // =========================
    // SELEZIONE TRIPLE (livello evento)
    // =========================
    bool triple = (arrayhitevent[0] == 1 && arrayhitevent[1] == 1 && arrayhitevent[2] == 1);
   ...
   // h.hQ2->Fill(b.pulses_integral[idx2]);
    ...
    h.mapdet3_afterselection->Fill(
        b.pulses_channel_x[idx3],
        b.pulses_channel_y[idx3]);
