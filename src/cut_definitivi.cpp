...
int main()
{
    ...
    TProfile *prof = new TProfile("prof", "Tempo medio per bin di ampiezza;Ampiezza;Tempo [ps]", 25, 0, 1);
...
timevsamplitude->SetPoint(timevsamplitude->GetN(), ampFEB[0], cfd60_co);
prof->Fill(ampFEB[0], cfd60[0]);
timevsintegral->SetPoint(timevsintegral->GetN(), integral[0], cfd60_co);
    if(integral[0]<40)
...
// 1. Definisci e Configura la Funzione Gaussiana (TF1)
TF1 *f_gaus = new TF1("f_gaus", "gaus", -1000, 500); // Definisci il nome, la formula, e il range iniziale del fit
...
cout<<" la sigma della gaussiana è: "<< sigma_12 <<endl;
...
cout<<"cavallo"<<endl;
  ...
  /*  if (sigma_2 > 0)
    ...
    //     Save everything
    // -----------------------
    TFile *fout = new TFile("muon_run_only2_cutdefinitivi.root", "RECREATE");
    hAmpAll->Write();
    hAmp1Hit->Write();
    hQ->Write();
    hTime20->Write();
