...
int main(int argc, char**argv) {
    ...
    int run = atoi(argv[1]);
    TString filename = Form("/home/riccardo-speziali/enubet/tb_april_26/sampic_run%d_merged.root", run);
    ...
    string outputFileName = Form("/home/riccardo-speziali/enubet/online_analysis/April_tb_online_run%d.root", run);

    // ------------------------------------------------
    //         Open input file and get tree and the mapping
    // ------------------------------------------------
   ...
   /* ifstream map();
    ...
    vector<hit_map> data_map;
    
    while(map >> detectors >> FEB >> channel >> x >> y){
        data_map.push_back({detectors, FEB, channel, x, y});
    }
    ...
    ProcessEvents(tree, branches, hists);
